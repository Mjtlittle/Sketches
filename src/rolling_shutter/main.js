...
function update_viewport() {
    ...
    if (image_comp != null) image_comp.resizeCanvas(target_width, target_height)
    else image_comp = createGraphics(target_width, target_height)
}
...
function setup() {
    ...
    settings = QuickSettings.create(20, 20, 'Rolling Shutter')
        .addDropDown(CAPTURE_MODE_LABEL, all_capture_modes, (v) => {
            set_capture_mode(v.value)
        })
        .addBoolean('Live Preview', show_preview, (v) => {
            show_preview = v
        })
        .addRange('Frame Rate [fps]', 0.02, 1, shutter_fps, 0.01, (v) => {
            shutter_fps = v
        })
        .addBoolean(PAUSE_LABEL, paused, set_pause)
        .addBoolean('Stop After Capture', stop_after_captured, (v) => {
            stop_after_captured = v
        })
...
function is_capture_vertical() {
    return (
        capture_mode == CaptureMode.TopToBottom ||
...
function add_frame_to_buffer() {
    ...
    if (!camera.loadedmetadata) return

    // capture a frame from the camera
    frame_buffer.push(camera.get())

    // limit the frame buffer size
    while (frame_buffer.length > slices) frame_buffer.shift()
    ...
    if (show_preview) {
        ...
        if (!paused) {
            ...
            if (is_capture_vertical()) {
                ...
                else oy = th - (th / slices) * frame_buffer.length
                ...
                else ox = tw - (tw / slices) * frame_buffer.length
